...
def post_apply_edits_dynamic(upload_resp: dict):
    ...
    for feat in features:
        adds.append({
            ...
            "SPKNumber": OUT_SPK,
            "KeyID": OUT_KEYID,
            "StartFlight": int(datetime.datetime.strptime(feat["attributes"].get("StartFlight", ""), "%Y-%m-%d %H:%M:%S").timestamp() * 1000) if feat["attributes"].get("StartFlight") else 0,
            "EndFlight": int(datetime.datetime.strptime(feat["attributes"].get("EndFlight", ""), "%Y-%m-%d %H:%M:%S").timestamp() * 1000) if feat["attributes"].get("EndFlight") else 0,
